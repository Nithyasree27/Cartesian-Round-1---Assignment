...
with customer_id as 
(select customer_id,row_number() over(order by customer_id) as rn_cust from customer),
voucher_id as 
(select voucher_id,row_number() over(order by voucher_id) as rn_voucher from voucher)
